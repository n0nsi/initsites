...
export async function validate(directory = resolve(root, 'dist')) {
  ...
  for (const file of found.filter(name => /\.(html|css|js|xml|txt|svg)$/.test(name) || name === '_headers')) {
    ...
    for (const ref of refs) {
      ...
      const path = posix.normalize(ref.startsWith('/') ? ref.slice(1) : posix.join(posix.dirname(file), ref.split(/[?#]/)[0]));

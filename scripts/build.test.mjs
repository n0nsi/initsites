...
async function fixture(t) {
  const dir = await mkdtemp(join(tmpdir(), 'initsites-build-test-'));
  t.after(() => rm(dir, { recursive: true, force: true }));
  for (const name of ['index.html', 'assets', 'public']) await cp(join(root, name), join(dir, name), { recursive: true });
  return dir;
}
...
test('reject an allowlisted source redirected through a symlink', async t => {
  const dir = await fixture(t);
  await rm(join(dir, 'assets/img/logo.svg'));
  await symlink(join(dir, 'index.html'), join(dir, 'assets/img/logo.svg'));
  await assert.rejects(build(dir), /Unsafe source path/);
});

test('reject a symlink output instead of deleting its target', async t => {
  const dir = await fixture(t);
  await symlink(join(dir, 'assets'), join(dir, 'dist'));
  await assert.rejects(build(dir), /dist must not be a symlink/);
});

test('reject contaminated output and unresolved dependencies', async t => {
  const dir = await fixture(t);
  ...
  await writeFile(join(output, 'assets/js/base.js'), "import missing from './missing.js';");
